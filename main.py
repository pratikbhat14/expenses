...
def get_expenses(b_file="Expenses.db", start_date="2020-05-31", end_date="2023-04-01"):
...
def calculate_categorical_expenses(df, key_dict, freq="1M"):
    if freq=="1M":
        strf_string = "%B-%Y"
    elif freq=="1Y":
        strf_string = "%Y"
...
if __name__=="__main__":
    ...
    df = get_expenses(db_file, start_date="2020-05-31", end_date="2023-04-01")
    df = categorize_expenses(df, key_dict=key_dict)

    print(df.head())

    result_monthly = calculate_categorical_expenses(df=df, key_dict=key_dict, freq="1M")
    result_yearly = calculate_categorical_expenses(df=df, key_dict=key_dict, freq="1Y")


    with open("monthly_expenses.json", "w") as f:
        json.dump(result_monthly, f, indent=4)

    with open("yearly_expenses.json", "w") as f:
        json.dump(result_yearly, f, indent=4)
